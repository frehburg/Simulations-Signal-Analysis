...
public class RunsUpTest {
    /**
     * This tests for the amount of unbroken subsequences of maximal length within which the U_i ’s
    ...
    public static boolean runsUpTest(UnifRandGenerator r, int n, double alpha) {
        ...
        System.out.println(Arrays.asList(runsUp));
        ...
        if(R < upperAlphaPercentile) {
            System.out.println("Found R="+R + " and chi squared (6df) =" + upperAlphaPercentile + ". Therefore we " +
                    "accept the hypothesis: the u are independent.");
            independent = true;
        } else {
            independent = false;
            System.out.println("Found R="+R + " and chi squared (6df) =" + upperAlphaPercentile + ". Therefore we " +
                    "reject the hypothesis: the u are not independent.");
        }
        return independent;
    }
}
